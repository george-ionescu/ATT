...
( function($) {
	...
	$.extend($.ui.gmap.prototype, {
            ...
            addMarkerWithLabel: function(markerOptions, callback) {
						...
						if ( status === 'OK' ) {
							response( $.map( results, function(item) {
								...
								if(item.types[0] == 'locality'){
									country = item.address_components[item.address_components.length-1].long_name;
								}else if(item.types[0] == 'point_of_interest'){
									country = item.address_components[item.address_components.length-2].long_name;
								}
								return { label: item.formatted_address, value: item.address_components[0].long_name, country: country, position: item.geometry.location, item:item}
							}));
